...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "BitcoinAdult" is a composite category enabling all BTAD-related debug output
            if (ptrCategory->count(std::string("BitcoinAdult"))) {
...
#else
    const char* pszModule = "BitcoinAdult";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BTAD
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BTAD
// Mac: ~/Library/Application Support/BTAD
// Unix: ~/.BitcoinAdult
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTAD";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BitcoinAdult.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty BitcoinAdult.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override BitcoinAdult.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BitcoinAdultd.pid"));
