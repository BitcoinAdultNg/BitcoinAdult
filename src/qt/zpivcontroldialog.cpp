    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zBTAD Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBTAD.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBTAD seed used to mint this zBTAD is not the same as currently hold in the wallet";
            else
