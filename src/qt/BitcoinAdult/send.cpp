...
#include "qt/BitcoinAdult/send.h"
#include "qt/BitcoinAdult/forms/ui_send.h"
#include "qt/BitcoinAdult/addnewcontactdialog.h"
#include "qt/BitcoinAdult/qtutils.h"
#include "qt/BitcoinAdult/sendchangeaddressdialog.h"
#include "qt/BitcoinAdult/optionbutton.h"
#include "qt/BitcoinAdult/sendconfirmdialog.h"
#include "qt/BitcoinAdult/myaddressrow.h"
#include "qt/BitcoinAdult/guitransactionsutils.h"
...
SendWidget::SendWidget(BITCOINADULTGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a BTAD address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BTAD");
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send BTAD");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zBTAD");
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zBTAD is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zBTAD selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zBTAD transaction sent!"));
        ...
        if (receipt.GetStatus() == ZPIV_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zBTAD require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBTAD transaction
        ...
        emit message("zBTAD transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isPIV){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zBTAD in your balance to select."));
        }
    }
}
