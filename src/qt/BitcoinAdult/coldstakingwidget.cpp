...
#include "qt/BitcoinAdult/forms/ui_coldstakingwidget.h"
#include "qt/BitcoinAdult/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/BitcoinAdult/requestdialog.h"
...
#include "qt/BitcoinAdult/csrow.h"
...
ColdStakingWidget::ColdStakingWidget(BITCOINADULTGUI* parent) :
    PWidget(parent),
    ui(new Ui::ColdStakingWidget)
{
    ...
    ui->labelSubtitle2->setText(tr("Delegate or Accept BTAD delegation"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BTAD outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 BTAD" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
