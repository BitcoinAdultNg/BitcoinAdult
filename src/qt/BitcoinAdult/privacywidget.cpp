...
#include "qt/BitcoinAdult/privacywidget.h"
#include "qt/BitcoinAdult/forms/ui_privacywidget.h"
#include "qt/BitcoinAdult/qtutils.h"
#include "guiutil.h"
#include "qt/BitcoinAdult/denomgenerationdialog.h"
#include "qt/BitcoinAdult/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(BITCOINADULTGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBTAD anonymizes your BTAD by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zBTAD or convert back to BTAD"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BTAD ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBTAD");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBTAD");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBTAD");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBTAD");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBTAD");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBTAD");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBTAD");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBTAD");
    ...
    // Only Convert to BTAD enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzPIV->setTitleClassAndText("btn-title-grey", "Total 0 zBTAD");
    ui->btnTotalzPIV->setSubTitleClassAndText("text-subtitle", "Show denominations of zBTAD owned.");
    ui->btnTotalzPIV->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BTAD outputs to mint into zBTAD.");
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBTAD");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BTAD to mint into zBTAD"));
    }else{
        btnText = tr("Convert back to BTAD");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBTAD to convert back into BTAD"));
    }
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBTAD is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zPIV, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBTAD").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBTAD minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zBTAD converted back to BTAD"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BTAD to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBTAD </b>";
