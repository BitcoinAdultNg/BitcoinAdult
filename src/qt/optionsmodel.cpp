...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BTAD);
    ...
    if (settings.contains("nAnonymizeBitcoinAdultAmount") || reset)
        SoftSetArg("-anonymizeBitcoinAdultamount", settings.value("nAnonymizeBitcoinAdultAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in BitcoinAdult.cpp during shotdown to also remove the window positions
